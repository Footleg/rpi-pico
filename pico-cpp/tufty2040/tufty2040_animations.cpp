/*
 * Example showing how you can run my hardware independent RGB LED animations
 * library on a display driven by an RP2040.
 *
...
uint16_t bounds_h = graphics.bounds.h;
uint16_t bounds_w = graphics.bounds.w;
...
class Animation : public RGBMatrixRenderer {
 public:
  Animation(uint8_t pixScale, uint16_t steps_, uint16_t minSteps_,
      ...
      : RGBMatrixRenderer{static_cast<uint16_t>(bounds_w / pixScale),
                          static_cast<uint16_t>(bounds_h / pixScale)},
  ...
  virtual void showPixels() { st7789.update(&graphics); }
 ...
 private:
  ...
  virtual void setPixel(uint16_t x, uint16_t y, RGB_colour colour) {
    ...
    if (pixelSize == 1) {
      ...
      graphics.circle(Point(x * pixelSize + rad + 1, y * pixelSize + rad + 1),
                      rad);
    } else {
      // Pixel sizes 4 or 5 draw four pixel block as circle does not draw nicely
      // at this size
      graphics.set_pixel(Point(x * pixelSize + 1, y * pixelSize + 1));
  ...
  while (true) {
    ...
    if (button_b.read()) {
      if (animationMode == animModeGol) {
        ...
        if (golStartPattern > 0 and pixelSize < 2) pixelSize = 2;
        ...
        animation.drawLine(animation.getGridWidth() / 2, yl, xl,
                           animation.getGridHeight() / 2);
        animation.drawLine(animation.getGridWidth() / 2,
                           animation.getGridHeight() - yl, xl,
                           animation.getGridHeight() / 2);
        animation.drawLine(animation.getGridWidth() / 2, yl,
                           animation.getGridWidth() - xl,
                           animation.getGridHeight() / 2);
        animation.drawLine(
            animation.getGridWidth() / 2, animation.getGridHeight() - yl,
            animation.getGridWidth() - xl, animation.getGridHeight() / 2);
